"""QM9 graph-regression datamodule using the cached incidence graphs."""
...
from cpen.apps.qm9.qm9_graph_cache import (
    MAX_NODES,
    CachedQM9GraphDataset,
    load_qm9_meta,
    missing_qm9_cache_message,
    qm9_cache_available,
    qm9_split_size,
    qm9_target_stats,
    resolve_target,
...
class QM9Datamodule(BaseDatamodule):
    """Molecular property regression on the RDKit bond graph.

    ``out_dim`` is 1: a single scalar per molecule. ``y`` is the raw target in
    its physical unit (dipole moment in Debye by default); ``target_mean`` /
    ``target_std`` are the train-split statistics the LightningModule uses to
    standardize it and to report MAE back in physical units.
    """
    ...
    def __init__(
        ...
        *,
        ...
        max_nodes = ((meta.get("splits") or {}).get("train") or {}).get("max_nodes")
        if max_nodes:
            self.n_particles = int(max_nodes)
        if meta.get("target_stats"):
            self.target_mean, self.target_std = qm9_target_stats(
                data_root, self.target_name
            )
    ...
    def _build_split(self, split: str, limit: int | None) -> Dataset:
        if not qm9_cache_available(self.data_root, split):
            raise FileNotFoundError(missing_qm9_cache_message(self.data_root))
        log_info(
            f"[graphs] split={split} mode=qm9-cache bonds+distance "
            f"target={self.target_name} [{self.target_unit}] "
            f"edge_features={self.include_edge_features}"
        )
        return CachedQM9GraphDataset(
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        if self.num_workers > 0:
            log_info(f"[graphs] QM9 mmap cache: num_workers={self.num_workers} with spawn")

    def _effective_size(self, split: str, limit: int | None) -> int:
        total = qm9_split_size(self.data_root, split)
