"""PascalVOC-SP node-classification datamodule using cached graph incidence."""
...
from cpen.utils.pascal_graph_cache import (
    PASCAL_SPLIT_SIZES,
    CachedPascalGraphDataset,
    missing_pascal_cache_message,
    pascal_cache_available,
)


class PascalVOCSPDatamodule(BaseDatamodule):
    N_FEATURES = 14
    N_EDGE_FEATURES = 2
    OUT_DIM = 21
    MAX_NODES = 500
    ...
    def _build_split(self, split: str, limit: int | None) -> Dataset:
        if not pascal_cache_available(self.data_root, split):
            raise FileNotFoundError(missing_pascal_cache_message(self.data_root))
        log_info(
            f"[graphs] split={split} mode=pascal-cache native edges "
            f"edge_features={self.include_edge_features}"
        )
        return CachedPascalGraphDataset(
            ...
            wire_coordinate_dim=self.wire_coordinate_dim,
            require_wire_cache=self.require_wire_cache,
        )
    ...
    def setup(self, stage: str | None = None) -> None:
        ...
        if self.num_workers > 0:
            log_info(f"[graphs] Pascal mmap cache: num_workers={self.num_workers} with spawn")

    @staticmethod
    def _effective_size(split: str, limit: int | None) -> int:
        total = PASCAL_SPLIT_SIZES[split]
