#!/bin/bash
# Resubmit L4 D=384 B=128×4 gpu-test chain until TARGET steps.
#
# Prefer:
#   bash scans/jets/start_L4D384_gputest_watchdogs.sh
# Or one arm:
#   ETA0=0.05 TARGET=100000 SLEEP_SEC=3600 nohup bash scans/jets/L4_D384_gputest_watchdog.sh &
#
set -uo pipefail
...
# η=0.1 keeps un-suffixed names (same convention as D1024 chain).
if [[ "$ETA_TAG" == "0p1" ]]; then
  JOB_NAME="${JOB_NAME:-jc-L4D384-gt}"
  RUN_TAG="${RUN_TAG:-L4D384-gputest-b128-g4}"
else
  JOB_NAME="${JOB_NAME:-jc-L4D384-gt-e${ETA_TAG}}"
  RUN_TAG="${RUN_TAG:-L4D384-gputest-b128-g4-e${ETA_TAG}}"
fi

ROOT=/scratch/gpfs/BHANIN/jdezoort/cpen_runs/jetclass_scaling_law
SLURM=/home/jdezoort/coupled-particle-edge-networks/scans/jets/scaling_law_L4_D384_b128_s100k_gputest.slurm
# Prefer scratch logs; fall back to home if scratch is not writable (agent/NFS squash).
LOG_CANDIDATES=(
  "$ROOT/logs/watchdog_L4D384_gputest_e${ETA_TAG}.log"
  "${HOME}/.cache/jc_L4D384_watchdogs/watchdog_L4D384_gputest_e${ETA_TAG}.log"
)
