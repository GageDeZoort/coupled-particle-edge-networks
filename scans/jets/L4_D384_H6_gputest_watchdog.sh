#!/bin/bash
# Resubmit L4 D=384 H=6 (d_head=64) B=128×4 gputest (59m) chain until TARGET steps.
#
# Prefer:
#   bash scans/jets/start_L4D384_H6_gputest_watchdogs.sh
# Or one arm:
#   ETA0=0.01 TARGET=100000 SLEEP_SEC=900 nohup bash scans/jets/L4_D384_H6_gputest_watchdog.sh &
#
set -uo pipefail
...
if [[ "$ETA_TAG" == "0p1" ]]; then
  JOB_NAME="${JOB_NAME:-jc-L4D384H6gt}"
  RUN_TAG="${RUN_TAG:-L4D384H6-gputest-b128-g4}"
else
  JOB_NAME="${JOB_NAME:-jc-L4D384H6gt-e${ETA_TAG}}"
  RUN_TAG="${RUN_TAG:-L4D384H6-gputest-b128-g4-e${ETA_TAG}}"
fi

ROOT=/scratch/gpfs/BHANIN/jdezoort/cpen_runs/jetclass_scaling_law
SLURM=/home/jdezoort/coupled-particle-edge-networks/scans/jets/scaling_law_L4_D384_H6_b128_s100k_gputest.slurm
LOG_CANDIDATES=(
  "$ROOT/logs/watchdog_L4D384H6_gputest_e${ETA_TAG}.log"
  "${HOME}/.cache/jc_L4D384_watchdogs/watchdog_L4D384H6_gputest_e${ETA_TAG}.log"
)
